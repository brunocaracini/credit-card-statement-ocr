...
class OcrEngine():
    ...
    def statement_orc_scanner_visa(self, pdf_path, bank='ICBC', entity='VISA'):
        ...
        for line in text.split('\n'):
            ...
            if entity == 'MASTERCARD' and not taxes_got:
                ...
                if self.is_tax() and not self.filter_tax_by_amount():
                    ...
                    item = Item(concept=concept,
                                ars_amount=ars_amount, type='taxes')
            ...
            if any(x in line.upper() for x in getattr(self, 'CARD_TOTAL')):
                ...
                items_set.card = Card(entity=entity, bank=bank, last_four_numbers=self.extract_last_four_numbers(
                )) if entity == 'VISA' else Card(entity=entity, bank=bank) if entity == 'MASTERCARD' else None
            ...
            if (date != False and
                ...
                if not self.is_tax(concept):
                    item = Item(date=date, concept=concept, ars_amount=ars_amount,
                                receipt=receipt, type='buy' if ars_amount >= 0 else 'bonus')
                    item.set_quotes_values_from_string(quote)
                else:
                    ars_amount = ars_amount if concept not in getattr(
                        self, 'IVA') and entity == 'VISA' else round(ars_amount * getattr(self, 'IVA_VALUE'), 2)
                    item = Item(date=date, concept=concept,
                                ars_amount=ars_amount, type='taxes')
    ...
    def extract_date(self):
        try:
            dates = [list(filter(re.compile(regex).match, self.list_splitted_item))[0] for regex in getattr(
                self, 'DATE_REGEX') if list(filter(re.compile(regex).match, self.list_splitted_item)) != []]
    ...
    def extract_quote(self):
        ...
        quote_prefixs = [x for word in getattr(self, 'QUOTE_PREFIX') for x in self.list_splitted_item if word in x.upper(
        ) and not re.search(getattr(self, 'QUOTE_REGEX'), x)]
        [self.list_splitted_item.remove(x) for x in quote_prefixs]

        if len(quote) > 0:
            self.list_splitted_item.remove(quote[0])
            prefixes = [prefix for prefix in getattr(
                self, 'QUOTE_PREFIX') if prefix in quote[0]]
    ...
    def exctract_receipt(self, entity):
        ...
        if len(receipts) > 0:
            self.list_splitted_item.remove(
                receipts[0]) if receipts[0] in self.list_splitted_item else None
        ...
        for element in self.list_splitted_item[::-1]:
            ...
            if self.is_number(number):
                ...
                if number[-1] == '-':
                    ...
                    number = number[1:]
                    negative = True
                return round(float(number) * -1, 2) if negative else round(float(number), 2)
    ...
    def is_tax(self, concept=None):
        if concept is None:
            concept = self.list_splitted_item[0] if len(
                self.list_splitted_item) > 1 else False
        if not concept:
            return False

        taxes = getattr(self, 'IMPUESTOS') + getattr(self, 'IVA') + \
    ...
    def multi_regex_match(self, attr, previous_date):
        ...
        value = self.list_splitted_item[0] if len(
            self.list_splitted_item) > 1 else ''
        if not any(re.match(regex, value) for regex in getattr(self, attr)):
            ...
            value = '-'.join(self.list_splitted_item[0:3]
                             ) if len(self.list_splitted_item) > 2 else ''
            if any(re.match(regex, value) for regex in getattr(self, attr)):
                ...
                return value
            elif day_number == self.extract_day_number_from_formated_date(formated_date=previous_date):
                self.list_splitted_item.remove(self.list_splitted_item[0])
                return previous_date
            elif (len(day_number) >= 2 and
                  self.is_number(value=day_number) and
                  int(day_number) <= 31):
                self.list_splitted_item.remove(self.list_splitted_item[0])
                return self.replace_day_number_in_formated_date(formated_date=previous_date, day_number=day_number)
            else:
                return False
        else:
