...
class DataItem(Data):
    ...
    def get_by_item_set(self, id_item_set: int | str):
        return [
            ...
            for res in super().select(
                table=self.TABLE_NAME, condition=f"id_item_set = {id_item_set}"
            )
        ]

    def insert(self, statement):
        values = [
            getattr(statement, col) for col in self.COLUMNS if getattr(statement, col)
        ]
    ...
    def export_to_excel(self, items: list[Item]):
        ...
        columns = [
            ...
            if getattr(items[0] if items[0].type != "bonus" else items[1], col)
        ]
